...
const Header: React.FC = () => {
  ...
  return (
    <header className="bg-white dark:bg-transparent">
      <nav
        className={`navbar navbar-expand-lg navbar-dark vj-pf-navbar ftco-navbar-light site-navbar-target p-4 ${isScrolled ? "scrolled" : ""
          } ${isAwake ? "awake" : "sleep"}`}
        id="ftco-navbar"
      >
              ...
              {headerLinks.map((link) => (
                <li key={link.href} className="nav-item">
                  <button
                    ...
                    className={`nav-link ${activeSection === link.href ? "active focus:bg-amber-100" : ""
                      } text-black hover:text-gray-900 dark:text-white dark:hover:text-white/75 cursor-pointer outline-none`}
                  >
...
const MusicVoulme = ({
  ...
  return (
    ...
    >
      ...
      <audio ref={audioRef} src="/audios/naruto_bgm.mp3" />
    </label>
  );
};
